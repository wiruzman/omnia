...
func TestStoreQueryIsCaseInsensitiveForDocker(t *testing.T) {
	...
	batch := []model.Entry{
		...
		{Path: "/Users/demo/Library/Containers/com.docker.docker", Name: "com.docker.docker", ParentPath: "/Users/demo/Library/Containers", RootPath: "/Users/demo", Type: model.TypeDirectory, Size: 10, CreatedAt: now, ModifiedAt: now},
	}
	if err := st.UpsertBatch(ctx, time.Now().UnixNano(), batch); err != nil {
		t.Fatal(err)
	}
